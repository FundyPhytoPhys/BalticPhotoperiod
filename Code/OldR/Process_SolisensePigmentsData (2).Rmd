...
MCPigmentExpFile <- "../Data/ProcessedData/ProcessedPigmentsData/Baltic_Photoperiod_Processed_PigmentAll.Rds"
...
SolFitsAllPigmentfitPhycoChlalinearExWL <- SolFitsAllPigmentPCPE %>%   
  select(c(Strain, Par_ue, Photoperiod, Ex_WL, meanSig_nm2psii, PhycoChlaRatioExp, O2, WL, Phase)) %>%  
...
SolFitsAllPigmentfitPhycoChlalinearStrain %>% 
  ...
  geom_line(aes(x = PhycoChlaRatioExp, y = `.fitted`), colour = "blue", linetype = "solid", size = 0.3, show.legend = F) +   
  scale_shape_manual(values = c(15, 16, 17,18), name="", labels = lab1) +
  scale_colour_manual(values = c("darkslategray", "lightblue4", "hotpink4", "indianred3", "goldenrod2", "khaki2"), name="", labels = lab2) +
...
#   filter(Phase=="Exponential") %>% 
#   filter(Strain == "PC-rich_056") %>% 
#   select(c(Sig, PhycoChlaRatioExp)) %>% 
#   rename(PigExp=PhycoChlaRatioExp)
# 
# SolFitsBA56St<-SolFitsAllPigmentPCPE %>% 
#   filter(Phase=="Pre-stationary") %>% 
#   filter(Strain == "PC-rich_056") %>% 
#   select(c(Sig, PhycoChlaRatioExp)) %>% 
#   rename(PigSt=PhycoChlaRatioExp)
# 
# SolFitsAllExp<-merge(SolFitsBA56Exp, SolFitsBA56St, by="Sig", all = T)
#   fit <- lm(Sig ~ ., data = SolFitsAllExp)
# anova(fit)
# 
#  diamonds.mod1 <- lm(Sig_nm2psii~PhycoChlaRatioExp, data = SolFitsAllExp)
#   diamonds.mod2 <- lm(Sig_nm2psii~PhycoChlaRatioExp+Strain, data = SolFitsAllSt)
