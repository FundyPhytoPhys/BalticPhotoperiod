...
PlotALightPUR<-ggplot(MCGrowthExpSP, aes(PURPhotonDose_day, deltaOD_Lmu_corr)) +
  ...
  scale_x_continuous(breaks=seq(0, 4e07, by = 1e07), label=scientific_10) +
  coord_cartesian(xlim = c(0, 4e07)) +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_GrowthRate_Light_PUR2",".png",sep = "")), height=9, width= 12,  dpi = 300, limitsize = TRUE)
```
...
LightPAR<-ggplot(MCGrowthSP, aes(PARPhotonDose_day, deltaOD_Lmu_corr)) +
  ...
  coord_cartesian(xlim = c(0, 9.5e07)) +
