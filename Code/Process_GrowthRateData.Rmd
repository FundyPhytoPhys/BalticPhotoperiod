...
Pigment exp do not contain 24 and 900uE data! Fix this Sylwia!!!!!
```{r}
# PigmentAllSmall<-PigmentAll %>% 
#   drop_na(PUR)
```
...
ggplot(MCGrowthExpSP, aes(PURPhotonDose_day, deltaOD_Lmu_corr)) +
  ...
  geom_line(aes(PURPhotonDose_day, .fitted), colour = "blue", linetype = "solid", size = 0.3, show.legend = F, GRCplatt_PredictGrowthExpAll) +
  scale_x_continuous(label=scientific_10) +
  ...
  scale_shape_manual(values = c(15, 16, 17, 18), name="", labels = lab1) +
  labs(y = "Chlorophyll specific exponential growth rate " ~ "("~d^-1~")", x = " PUR photon dose ( µmol photons "~m^-2~""~d^-1~")") +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("SFig_GrowthRate_PUR",".png",sep = "")), height=9, width= 7,  dpi = 300, limitsize = TRUE)
```
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("SFig_GrowthRate_Light",".png",sep = "")), height=9, width= 7,  dpi = 300, limitsize = TRUE)
```
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_GrowthRate_Photoperiod",".png",sep = "")), height=9, width= 7,  dpi = 300, limitsize = TRUE)
```
