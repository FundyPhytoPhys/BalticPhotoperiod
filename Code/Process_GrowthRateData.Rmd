...
PlotALightPUR<-ggplot(MCGrowthExpSP, aes(PURPhotonDose_day, deltaOD_Lmu_corr)) +
  ...
  geom_text(data=data_textA, aes(x=0, y=4.3, label=label), size=5) +
  scale_x_continuous(label=scientific_10) +
...
```{r save plot}
# ggsave(file = file.path(FigPath, paste("SFig_GrowthRate_Light_PUR",".png",sep = "")), height=9, width= 7,  dpi = 300, limitsize = TRUE)
```
...
PlotB<-MCParamAll_Photoperiod %>% 
  ...
  geom_text(data=data_textB, aes(x=8, y=1.5e-06, label=label), size=5) +
  ...
  coord_cartesian(ylim = c(0, 1.6e-06)) +
  labs(y = "Alpha", x = "Photoperiod (h)") +
  ggh4x::facet_nested(rows = vars(facetsStrain, Strain), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
...
PlotB<-MCParamAll_Light %>% 
  ...
  geom_line(aes(x = as.numeric(Fit), y = estimate), linetype = "dashed", size = 0.5, alpha = 0.8, show.legend = F) +
  geom_errorbar(aes(x = as.numeric(Fit), ymin = estimate - SE, ymax = estimate + SE), width=0, size=0.3, data = . %>% filter(SE<2.997371e+01), show.legend = F) +
  geom_text(data=data_textB, aes(x=30, y=1.4e-06, label=label), size=5) +
  ...
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="") +
  scale_y_continuous(breaks=seq(0, 1.5e-06, by = 0.5e-06), label=scientific_10) +
  scale_x_continuous(limits = c(30, 900), breaks = c(30, 90, 180, 300, 600, 900)) +
  coord_cartesian(ylim = c(-0.2e-06, 1.5e-06)) +
  labs(y = "Alpha", x = "PAR ( µmol photons "~m^-2~""~s^-1~")") +
  ggh4x::facet_nested(rows = vars(facetsStrain, Strain), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
...
```{r preliminary plot, warning = FALSE, fig.height = 5, fig.width = 5}
scientific_10 <- function(y) {   parse(text=gsub("e\\+*", " %*% 10^", scales::scientific_format()(y))) }
data_textA <- data.frame(label = c('A'))
PlotA<-MCParamAll_Light %>% 
  filter(term == "a") %>% 
  filter(Fit!= "All") %>% 
  rename(SE="std.error") %>% 
  ggplot() +
  geom_point(aes(x = as.numeric(Fit), y = estimate, color = as.factor(Strain)), size = 2.5, alpha = 0.8, show.legend = T) +
  geom_smooth(aes(x = as.numeric(Fit), y = estimate, color = as.factor(Strain)), linetype = "dotted", size = 0.5, alpha = 0, show.legend = F) +
  # geom_errorbar(aes(x = as.numeric(Fit), ymin = estimate - SE, ymax = estimate + SE, colour = as.factor(Strain)), width=0, size=0.3, data = . %>% filter(SE<2.997371e+01), show.legend = F) +
  geom_text(data=data_textA, aes(x=9, y=8e-07, label=label), size=5) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="") +
  scale_y_continuous(breaks=seq(0, 8e-07, by = 2e-07), label=scientific_10) +
  coord_cartesian(ylim = c(0, 8e-07)) +
  # labs(y = "Alpha ( µmol photons "~m^-2~""~d^-1~")") +
  labs(y = "Alpha") +
...
PlotB<-MCParamAll_Photoperiod %>% 
  ...
  rename(SE="std.error") %>% 
  ggplot() +
  geom_point(aes(x = as.numeric(Fit), y = estimate, color = as.factor(Strain)), size = 2.5, alpha = 0.8, show.legend = F) +
  geom_smooth(aes(x = as.numeric(Fit), y = estimate, color = as.factor(Strain)), linetype = "dotted", size = 0.5, alpha = 0, show.legend = F) +
  # geom_errorbar(aes(x = as.numeric(Fit), ymin = estimate - SE, ymax = estimate + SE, colour = as.factor(Strain)), width=0, size=0.3, data = . %>% filter(SE<2.997371e+01), show.legend = F) +
  geom_text(data=data_textB, aes(x=8, y=8e-07, label=label), size=5) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="") +
  scale_y_continuous(breaks=seq(0, 8e-07, by = 2e-07), label=scientific_10) +
  scale_x_continuous(limits = c(8, 24), breaks = c(8, 12, 16, 24)) +
  coord_cartesian(ylim = c(0, 8e-07)) +
...
PlotC<-MCParamAll_Light %>% 
  filter(term == "Pmax") %>% 
  filter(Fit!= "All") %>% 
  rename(SE="std.error") %>% 
  #filter(SE<5.655529e+05) %>% 
  ggplot() +
  geom_point(aes(x = as.numeric(Fit), y = estimate, color = as.factor(Strain)), size = 2.5, alpha = 0.8, show.legend = F) +
  geom_smooth(aes(x = as.numeric(Fit), y = estimate, color = as.factor(Strain)), linetype = "dotted", size = 0.5, alpha = 0, show.legend = F) +
  # geom_errorbar(aes(x = as.numeric(Fit), ymin = estimate - SE, ymax = estimate + SE, colour = as.factor(Strain)), width=0, size=0.3, show.legend = F) +
  geom_text(data=data_textC, aes(x=9, y=4000, label=label), size=5) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="") +
  scale_y_continuous(label=scientific_10) +
  # labs(y = ""~italic(P)~""["max"]~" ( µmol photons "~m^-2~""~d^-1~")", x = "PAR ( µmol photons "~m^-2~""~s^-1~")") +
  labs(y = ""~italic(P)~""["max"]~"", x = "PAR ( µmol photons "~m^-2~""~s^-1~")") +
  # scale_y_continuous(breaks=seq(0, 5e-07, by = 1e-07), label=scientific_10) +
  # coord_cartesian(ylim = c(0, 5e-07)) +
...
PlotD<-MCParamAll_Photoperiod %>% 
  ...
  geom_smooth(aes(x = as.numeric(Fit), y = estimate, color = as.factor(Strain)), linetype = "dotted", size = 0.5, alpha = 0, show.legend = F) +
  # geom_errorbar(aes(x = as.numeric(Fit), ymin = estimate - SE, ymax = estimate + SE, colour = as.factor(Strain)), width=0, size=0.3, data = . %>% filter(SE<2.997371e+01), show.legend = F) +
  geom_text(data=data_textD, aes(x=8, y=4000, label=label), size=5) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="") +
  scale_y_continuous(breaks=seq(0, 4000, by = 1000), label=scientific_10) +
  scale_x_continuous(limits = c(8, 24), breaks = c(8, 12, 16, 24)) +
...
PARPhotoperiod<-ggplot(MCGrowthSP, aes(PARPhotonDose_day, deltaOD_Lmu_corr)) +
  geom_point(aes(PARPhotonDose_day, deltaOD_Lmu_corr, colour = as.factor(Par_ue), shape = as.factor(Photoperiod)), alpha = 0.8, size = 3.5, show.legend = T) +
  geom_errorbar(aes(x = PARPhotonDose_day, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se, colour = as.factor(Par_ue)), width=0, size=0.3, data = . %>% filter(deltaOD_Lmu_se<8.057558e-03), show.legend = F) +
  geom_line(aes(PARPhotonDose_day, .fitted), colour = "blue", linetype = "solid", size = 0.3, show.legend = F, GRCplatt_PredictGrowthAll) +
  geom_line(aes(PARPhotonDose_day, .fitted), colour = "black", linetype = "dotted",  show.legend = F, GRCplatt_PredictGrowth8) +
...
ParamPAR<-MCParamAll_Photoperiod %>% 
  filter(term == "a") %>% 
  filter(Fit!= "All") %>% 
  ggplot() +
  geom_point(aes(x = as.numeric(Fit), y = estimate, color = Strain), show.legend = F) +
  ggh4x::facet_nested(rows = vars(Strain), labeller = label_parsed) +
  theme_bw()
ParamPAR
```
```{r, warning = FALSE, fig.height = 9, fig.width = 10}
ggp<- PARPhotoperiod+ParamPAR   
ggp                           
```
