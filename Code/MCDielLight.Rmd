...
MCData <- MCData |>
  ...
  filter(deltaOD > 0) |>
  mutate(Actinic_photonsm2h = zoo::rollsum(Actinic_par * 5, k = 12, fill = NA, align = "right")) |> # will not work b/c will sum across entire df
...
MCData |>
  ...
  geom_line(aes(x = Actinic_photonsm2h, y = mu_Averh, colour = ToD)) +
  facet_grid(cols = vars(Tube)) +
  theme_bw()
...
MCData |>
  ...
  geom_point(aes(x = ToD, y = mu_Averh, colour = Actinic_photonsm2h)) +
  geom_line(aes(x = ToD, y = mu_Averh, colour = Actinic_photonsm2h)) +
  geom_point(aes(x = ToD, y = Actinic_photonsm2h/100000, colour = Actinic_photonsm2h)) +
  facet_grid(cols = vars(Tube)) +
  theme_bw()
