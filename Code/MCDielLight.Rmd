...
MCDataHour <- MCDataAll |>
  ...
  #filter(deltaOD > 0) |> issue with filter for estimation of Actinic_photonsm2h
  mutate(Actinic_photonsm2h = zoo::rollsum(Actinic_par * 5 * 60, k = 60/ODinterval, fill = NA, align = "right")) |> #PAR per h
  mutate(mu_deltaODwindow = zoo::rollapply(cbind(time, log(deltaOD)), width = 60/ODinterval, by = 1, slope, by.column=FALSE, fill=NA)) |> #slope
  mutate(mu_Averh = zoo::rollmean(mu_deltaODwindow, k = 60/ODinterval, fill = NA, align = "right")) |> #average of first 12 row
  ungroup() |>
...
```{r cumulative diel PAR}
MCDataHour <- MCDataHour |>
  group_by(Filename, Day, Tube) |>
  mutate(Actinic_photonsm2d = sum(Actinic_photonsm2h, na.rm = TRUE)) |>
  ungroup()

MCDataHour |>
  filter(Run == 39) %>% 
  ggplot() +
  geom_point(aes(x = PARPhotonDose_day, y = Actinic_photonsm2d)) +
  geom_line(aes(x = PARPhotonDose_day, y = Actinic_photonsm2d)) +
  facet_grid(rows = vars(Strain, Photoperiod), cols = vars(Tube)) +
  theme_bw()
...
MCDataHourSVR <- MCDataHour %>%
  select(c(time, ToD, Day, mu_deltaODwindow, mu_Averh, Actinic_photonsm2h, Actinic_photonsm2d, SampleID, Strain, Tube, Run, Par_ue, Photoperiod, OD720, OD680, deltaOD, Actinic_par, WL, O2)) %>%
...
MCImportAll %>%
 ...
 #filter(time >48) %>% 
  filter(Run != 77) %>% 
  ggplot() +
  geom_point(aes(x = time, y = mu_deltaODwindow, colour = as.factor(Strain)), alpha = 0.2, show.legend = F) +
  geom_line(aes(x = time, y = svr_pred, colour = as.factor(Strain)), size = 0.6, show.legend = T) +
    geom_vline(aes(xintercept = 1 * 24), linetype = "dotted", alpha = 0.4) + 
    geom_vline(aes(xintercept = 2 * 24), linetype = "dotted", alpha = 0.4) + 
    geom_vline(aes(xintercept = 3 * 24), linetype = "dotted", alpha = 0.4) + 
    geom_vline(aes(xintercept = 4 * 24), linetype = "dotted", alpha = 0.4) + 
    ...
    geom_vline(aes(xintercept = 13 * 24), linetype = "dotted", alpha = 0.4) +
    geom_vline(aes(xintercept = 14 * 24), linetype = "dotted", alpha = 0.4) +
  scale_colour_discrete(type=c("seagreen4"), name="") +
...
MCImportAll %>%
 ...
 #filter(time >48) %>% 
  ...
  geom_line(aes(x = time, y = svr_pred, colour = as.factor(Strain)), size = 0.6, show.legend = T) +
    ...
    geom_vline(aes(xintercept = 13 * 24), linetype = "dotted", alpha = 0.4) +
    geom_vline(aes(xintercept = 14 * 24), linetype = "dotted", alpha = 0.4) +
  scale_colour_discrete(type=c("palegreen3"), name="") +
  scale_y_continuous(breaks=seq(-0.5, 0.5, by = 0.25)) +
  coord_cartesian(ylim = c(-0.5, 0.5)) +
  #labs(y = "Optical density ("~OD[680]~")", x = "Elapsed time (h)") +
  ggh4x::facet_nested(cols = vars(facetsPhotoperiod, Photoperiod), rows = vars(facetsPar_ue, Par_ue), labeller = label_parsed) +
...
MCImportAll %>%
 ...
 #filter(time >48) %>% 
  ...
  geom_line(aes(x = time, y = svr_pred, colour = as.factor(Strain)), size = 0.6, show.legend = T) +
    ...
    geom_vline(aes(xintercept = 13 * 24), linetype = "dotted", alpha = 0.4) +
    geom_vline(aes(xintercept = 14 * 24), linetype = "dotted", alpha = 0.4) +
  scale_colour_discrete(type=c("brown1"), name="") +
  scale_y_continuous(breaks=seq(-0.5, 0.5, by = 0.25)) +
  coord_cartesian(ylim = c(-0.5, 0.5)) +
  #labs(y = "Optical density ("~OD[680]~")", x = "Elapsed time (h)") +
  ggh4x::facet_nested(cols = vars(facetsPhotoperiod, Photoperiod), rows = vars(facetsPar_ue, Par_ue), labeller = label_parsed) +
...
MCImportAll %>%
 ...
 #filter(time >48) %>% 
  ...
  geom_line(aes(x = time, y = svr_pred, colour = as.factor(Strain)), size = 0.6, show.legend = T) +
    ...
    geom_vline(aes(xintercept = 13 * 24), linetype = "dotted", alpha = 0.4) +
    geom_vline(aes(xintercept = 14 * 24), linetype = "dotted", alpha = 0.4) +
  scale_colour_discrete(type=c("brown4"), name="") +
  scale_y_continuous(breaks=seq(-0.5, 0.5, by = 0.25)) +
  coord_cartesian(ylim = c(-0.5, 0.5)) +
  #labs(y = "Optical density ("~OD[680]~")", x = "Elapsed time (h)") +
  ggh4x::facet_nested(cols = vars(facetsPhotoperiod, Photoperiod), rows = vars(facetsPar_ue, Par_ue), labeller = label_parsed) +
