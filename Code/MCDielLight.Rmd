...
MCData <- MCData %>%
  mutate(mu_deltaODwindow = rollapply(cbind(time, log(deltaOD)), 12, slope, by.column=FALSE, fill=NA)) |>
...
MCData |>
  filter(time >= 48,
         time <= 72) |>
  ggplot() +
  geom_point(aes(x = time, y = mu_deltaODwindow, colour = WL)) +
  geom_point(aes(x = time, y = mu_deltaODwindow)) +
  facet_grid(cols = vars(Tube)) +
  theme_bw()
```
