...
data_text_AnovaSt<- data.frame(WL = c("WW", "WW"), Phase = c("Pre-stationary"), O2=c(21, 21, 21, 21), Strain  = c("PC-rich_056", "PC-rich_077", "PE-rich_048", "PE-rich_127"),  label = c('ANOVA, p < 0.05, a', 'ANOVA, p < 0.05, ab', 'ANOVA, p < 0.05, ab', 'ANOVA, p < 0.05, b'))
...
lab2=c(expression("30 µE"), expression("90 µE"), expression("180 µE"), expression("300 µE"), expression("900 µE"))
  ...
  geom_text(data=data_text_AnovaExp, aes(x=4500000, y=0, label=label), colour = "blue", size=3.0) +
  geom_text(data=data_text_AnovaSt, aes(x=4500000, y=0, label=label), colour = "blue", size=3.0) +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_GS",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
...
fittedmodelPhase<-fittedmodel 
  ...
  # anovaStrainAll<-anova(fittedmodelStrain[[3]][[7]], fittedmodelStrain[[3]][[8]], fittedmodelStrain[[3]][[6]], fittedmodelStrain[[3]][[5]])
...
```{r}
saveRDS(AnovaTest_GS_Param, file.path(TableRdsPath, paste(Project, "Anova_GS.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_AccLen",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("SFig_AccLen",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_TDG",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
...
saveRDS(fitted_GS_Param, file.path(FigRdsPath, paste(Project, "Fig_GrowthSymmetry_2.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
