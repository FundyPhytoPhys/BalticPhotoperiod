...
lab2=c(expression("30 µE"), expression("90 µE"), expression("180 µE"), expression("300 µE"), expression("900 µE"))
  ...
  geom_text(data=data_text_AnovaPhase, aes(x=9000000, y=-0.2, label=label), colour = "blue", size=4) +
...
AnovaTest_GS<-data.frame(unclass(summary(model)), check.names = FALSE, stringsAsFactors = FALSE)
...
```{r}
saveRDS(AnovaTest_GS, file.path(TableRdsPath, paste(Project, "Anova_GS.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
...
# Perform t-test
t_test_result <- t.test(coef_model1, coef_model2)

# Extract relevant information
result_data <- data.frame(
  t_test = t_test_result$statistic,
  ...
  Fit_model = c("Model1", "Model2")  # Adjust as needed
)
...
fittedmodelStrainExp<-fittedmodel 
  ...
  fittedmodelStrainExp<-fittedmodel 
  anova056077Exp<-t.test(fittedmodelStrainExp[[3]][[7]], fittedmodelStrainExp[[3]][[8]])
  anova056077Exp$FitComparison <- '056_077_Exp'
  anova048127Exp<-t.test(fittedmodelStrainExp[[3]][[6]], fittedmodelStrainExp[[3]][[5]])
  anova048127Exp$FitComparison <- '048_127_Exp'
  anova056048Exp<-t.test(fittedmodelStrainExp[[3]][[7]], fittedmodelStrainExp[[3]][[6]])
  anova056048Exp$FitComparison <- '056_048_Exp'
  anova077048Exp<-t.test(fittedmodelStrainExp[[3]][[8]], fittedmodelStrainExp[[3]][[6]])
  anova077048Exp$FitComparison <- '077_048_Exp'
  anova056127Exp<-t.test(fittedmodelStrainExp[[3]][[7]], fittedmodelStrainExp[[3]][[5]])
  anova056127Exp$FitComparison <- '056_127_Exp'
  anova077127Exp<-t.test(fittedmodelStrainExp[[3]][[8]], fittedmodelStrainExp[[3]][[5]])
  anova077127Exp$FitComparison <- '077_127_Exp'
  anovaStrainExp<-rbind(anova056077Exp, anova048127Exp, anova056048Exp, anova077048Exp, anova056127Exp, anova077127Exp)
  ttestStrainExp <-anovaStrainExp %>% 
  drop_na(Df) 
  rm(anova056077Exp, anova048127Exp, anova056048Exp, anova077048Exp, anova056127Exp, anova077127Exp)
...
GScatalogMeta %>%
  ...
  guides(color = guide_legend(order = 1), shape  = guide_legend(order = 0)) +
  labs(y = "Hours of photoperiod to reach maximum daily growth (h)", x = "Photon dose ( µmol photons "~m^-2~""~d^-1~")") +
...
GScatalogMeta %>%
  ...
  guides(color = guide_legend(order = 1), shape  = guide_legend(order = 0)) +
  labs(y = "Hours of photoperiod to reach maximum daily growth (h)", x = "Photoperiod (h)") +
  ggh4x::facet_nested(cols = vars(facetsPhase, Phase), rows = vars(facetsStrain, Strain), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
...
# Calculated Anova and Tukey test from AccLen data points

```{r calculated statistics}
GScatalogMetaAccLenStats<-GScatalogMeta 
  #filter(Par_ue != 600) 
...
# Three way Anova with interactions
model<-aov(AccLen~PARPhotonDose_day*Phase*Strain, data=GScatalogMetaFitStats)
AnovaTest_AccLen<-data.frame(unclass(summary(model)), check.names = FALSE, stringsAsFactors = FALSE)
TukeyHSDTest_AccLen<-TukeyHSD(model, which = c("PARPhotonDose_day", "Phase", "Strain"))
```
