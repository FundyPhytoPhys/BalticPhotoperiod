...
# List processed Olis and Jaz files

```{r exported Rmd only first time in session}
list.files(path = DataIn, pattern = Project, full.names = TRUE)
```
# Read processed Olis and Jaz files
...
rm(PigmentAll, MCGrowthRate)
```

# Create preliminary plot PAR photodose
...
```{r platt fit, warning = FALSE, echo = FALSE}
#Harrison & Platt, 1986
...
# Create preliminary plot - per Par_ue and photoperiod and all fits (without stats)
...
# We can make ANOVA fromm all models, but then we don't know what is significantky different
# MCModelAll_Test<-MCModelAll_Light
#   anova_Test<-anova(MCModelAll_Test[[2]][[1]], MCModelAll_Test[[2]][[2]], MCModelAll_Test[[2]][[3]], MCModelAll_Test[[2]][[4]], MCModelAll_Test[[2]][[5]], MCModelAll_Test[[2]][[6]])
...
ggplot(MCGrowthSP, aes(PARPhotonDose_day, deltaOD_Lmu_corr)) +
  ...
  scale_colour_manual(values = c("darkslategray", "lightblue4", "hotpink4", "indianred3", "goldenrod2", "khaki2"), name="", labels = lab2, breaks=c('30', "90", "180", "300", "600","900")) +
  scale_shape_manual(values = c(15, 16, 17, 18), name="", labels = lab1) +
  labs(y = "Chlorophyll specific exponential growth rate " ~ "("~d^-1~")", x = "Photon dose ( µmol photons "~m^-2~""~d^-1~")") +
  ggh4x::facet_nested(rows = vars(facetsStrain, Strain), labeller = label_parsed) +
  theme_bw() +
...
```{r}
rm(MCGrowthSPAllModel, MCGrowthSP30Model, MCGrowthSP90Model, MCGrowthSP180Model, MCGrowthSP300Model, MCGrowthSP900Model, MCGrowthSP8Model, MCGrowthSP12Model, MCGrowthSP16Model, MCGrowthSP24Model, GRCplatt_PredictGrowth30, GRCplatt_PredictGrowth90, GRCplatt_PredictGrowth180, GRCplatt_PredictGrowth300, GRCplatt_PredictGrowth900, GRCplatt_PredictGrowth8, GRCplatt_PredictGrowth12, GRCplatt_PredictGrowth16, GRCplatt_PredictGrowth24, GRCplatt_PredictGrowth900_BA127, GRCplatt_PredictGrowth900_BA48, GRCplatt_PredictGrowth900_BA56, GRCplatt_PredictGrowth900_BA77, MCGrowthSP, GrowthRateAllParam, MCGrowthRateStats)
```


# Other models and stats - examples:
...
# Example of t-test from the linear model 
