...
Issue - too ma y points, code works too slow.
I filter growth rate per h during the night - when I do not collect solisense data for sure

But it is not enought - I need to merge by time - to bet the same time when I measured Solisense and growth rate per h !!!! Fix this Sylwia!!!!!!!

```{r}
GrowthRateAllChoosen<-GrowthRateAll %>% 
  filter(ToD >= 7) %>% 
  filter(ToD <= 20)
```
...
```{r Joining growth rate and Solisense exp data}
GrowthRateSoliExp <- GrowthRateAllChoosen %>% 
  left_join(., Solisense, by = c("SampleID"="SampleID", "Run"="Run",  "Strain"="Strain", "ExpDate"="ExpDate", "Par_ue"="Par_ue","Photoperiod"="Photoperiod", "O2"="O2", "WL"="WL", "PARPhotonDose_day"="PARPhotonDose_day", "Day" = "E_days")) 
