...
```{r create plot}
GRCplatt_PredictSolFits1<-GRCplatt_PredictSolFits %>% #only for prelim plot
  filter(Strain == "BA48R") %>% 
  filter(Par_ue == 90) %>% 
...
```{r save rds}
# saveRDS(SolFitsMeta, file.path(DataOut, paste(Project, "Imported_Solisense_LRC.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
