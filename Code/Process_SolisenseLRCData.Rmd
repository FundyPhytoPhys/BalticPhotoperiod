...
ggplot() +
  ...
  #geom_text(data=data_textB, aes(x=800, y=0.7, label=label), size=4.5) +
...
# Trzeba zrobic Platt dla JVPSII per chl lub cell!
wpierw podzielic jvpsii per chla lub cell potem zrobic plat!

```{r}
# colnames(SolFitsTurnerMDPico)

SolFitsTurnerMDPicoPlot<-SolFitsTurnerMDPico %>% 
  select(c(SampleID, meanTurChl_ugL, Strain, Par_ue, Photoperiod, Ex_WL, E_days, Run, meanCellmL_MDPico, JVPSII_umol_eLs_Par_ue, JVPSII_umol_eLs_HalfPar_ue)) %>% 
  mutate(meanCellL_MDPico=meanCellmL_MDPico*1000) %>% 
  mutate(JVPSII_umol_ecells_Par_ue=JVPSII_umol_eLs_Par_ue/meanCellL_MDPico) %>% 
  mutate(JVPSII_umol_eChlas_Par_ue=JVPSII_umol_eLs_Par_ue/meanTurChl_ugL) %>% 
...
SolFitsMetaLRCPlot<-SolFitsMetaLRC %>% 
  mutate(Strain=case_when(Strain=="BA127R"~"PE-rich_127",
         Strain=="BA48R"~"PE-rich_048",
        Strain=="BA56G"~"PC-rich_056",
         Strain=="BA77G"~"PC-rich_077")) 
...
```{r}
MDPicoMetaPlot <- SolFitsMetaLRCPlot %>%
  left_join(., SolFitsTurnerMDPicoPlot, by = c("SampleID" = "SampleID", "Strain"="Strain", "Par_ue"="Par_ue", "Photoperiod"="Photoperiod", "E_days"="E_days", "Run"="Run", "Ex_WL"="Ex_WL")) %>% 
  drop_na(Strain) %>% 
  filter(Par_ue == 90) %>% 
  filter(Photoperiod == 16) %>% 
  filter(E_days == 7) %>% 
  drop_na(JVPSII_umol_ecells_Par_ue) %>% 
  drop_na(JVPSII_umol_eChlas_Par_ue) 
```
...
Plot<-MDPicoMetaPlot %>%  
  ...
  filter(Photoperiod == 16) %>%
  filter(E_days == 7) %>% 
  select(c(SampleID, Run, Strain, Par_ue, Photoperiod, Ex_WL, E_days, ActPARCorr, ".fitted", JVPSII_umol_eLs_Par_ue, JVPSII_umol_ecells_Par_ue, JVPSII_umol_eChlas_Par_ue, facetsExcitation)) %>% 
  unique()
