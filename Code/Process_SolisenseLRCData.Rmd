...
Import_SolisenseDataLRC.Rmd imports BalticPhotoperiod_Imported_SolisenseLight_LRC.Rds. 
...
ggplot() +
  ...
  geom_line(aes(ActPARCorr, .fitted*100), colour = "black", linetype = "dotted") +
  
  geom_text(data=data_textB, aes(x=800, y=450, label=label), size=4.5) +
  labs(y = ""~italic(JV)~""["PSII"]*" ("~e^-~""~L^-1~""~s^-1~")", x = "Actinic PAR ( µmol photons "~m^-2~""~s^-1~")") +
...
ss<-ggarrange(PlotA,PlotB,
          ncol = 1, nrow = 2) %>% 
  ggsave(file = file.path(FigPath, paste("Fig_LIFT_2",".png",sep = "")), height=9, width= 7,  dpi = 300, limitsize = TRUE)
```
...
--------------------------------------# Choose cooresponding PAR for Sig  ----------------------------------------------------

# Choosen Sigma from corresponding light

```{r data taken from corresponding light calibration}
SolFitsTrimMetaClean30 <- SolFitsMetaRaw %>%
  filter(Dark1s == 0) %>%
  filter(Par_ue == 30) %>%
  filter(ActPAR == 20) # 39.5 for 445 and 48.7 for 590

SolFitsTrimMetaClean90 <- SolFitsMetaRaw %>%
  filter(Dark1s == 0) %>%
  filter(Par_ue == 90) %>%
  filter(ActPAR == 40) # 79.0 for 445 and 97.5 for 590

SolFitsTrimMetaClean180 <- SolFitsMetaRaw %>%
  filter(Dark1s == 0) %>%
  filter(Par_ue == 180) %>%
  filter(ActPAR == 80) # 158.1 for 445 and 195.0 for 590

SolFitsTrimMetaClean300 <- SolFitsMetaRaw %>%
  filter(Dark1s == 0) %>%
  filter(Par_ue == 300) %>%
  filter(ActPAR == 160) # 316.2 for 445 and 390.0 for 590

SolFitsTrimMetaClean600 <- SolFitsMetaRaw %>%
  filter(Dark1s == 0) %>%
  filter(Par_ue == 600) %>%
  filter(ActPAR == 320) # 632.5 for 445 and 780.1 for 590

SolFitsTrimMetaClean900 <- SolFitsMetaRaw %>%
  filter(Dark1s == 0) %>%
  filter(Par_ue == 900) %>%
  filter(ActPAR == 320) # 632.5 for 445 and 780.1 for 590

SolFitsTrimMetaClean<-rbind(SolFitsTrimMetaClean30, SolFitsTrimMetaClean90, SolFitsTrimMetaClean180, SolFitsTrimMetaClean300, SolFitsTrimMetaClean600, SolFitsTrimMetaClean900)
...
SolFitsTrimMetaDarkafterLight30 <- SolFitsMetaRaw %>%
  filter(Dark1s == 1) %>%
  filter(ActPAR == 0) %>%
  filter(Par_ue == 30) %>%
...
SolFitsTrimMetaDarkafterLight90 <- SolFitsMetaRaw %>%
  filter(Dark1s == 1) %>%
  filter(ActPAR == 0) %>%
  filter(Par_ue == 90) %>%
...
SolFitsTrimMetaDarkafterLight180 <- SolFitsMetaRaw %>%
  filter(Dark1s == 1) %>%
  filter(ActPAR == 0) %>%
  filter(Par_ue == 180) %>%
...
SolFitsTrimMetaDarkafterLight300 <- SolFitsMetaRaw %>%
  filter(Dark1s == 1) %>%
  filter(ActPAR == 0) %>%
  filter(Par_ue == 300) %>%
...
SolFitsTrimMetaDarkafterLight900 <- SolFitsMetaRaw %>%
  filter(Dark1s == 1) %>%
  filter(ActPAR == 0) %>%
  filter(Par_ue == 900 | Par_ue == 600) %>%
...
rm(CultureCatalog, ActPARCrossCal)
```
...
```{r}
colnames(SolFitsTrimMetaDarkafterLight)
```
