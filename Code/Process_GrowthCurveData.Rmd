...
MultiCultiDataAll %>%
  ...
  filter(Run != 77) %>% 
  ggplot() +
  geom_area(aes(x = Time_h, y = meanActinic_par_h/2), size = 0.1, fill = "tan1", alpha = 0.3) +
...
MCGrowthRateFitData %>%
  ...
  ggplot() +
  #geom_area(aes(x = time_h, y = meanActinic_par_h), size = 0.1, fill = "tan1", alpha = 0.6) +
  geom_line(aes(x = time_h, y = meanDeltaOD_h, colour = as.factor(Strain)), size = 0.4, alpha = 0.5, show.legend = F) +
  geom_line(aes(x = time_h, y = meanfittedDeltaOD_h, colour = as.factor(Strain)), size = 0.8, alpha = 0.9, show.legend = T) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="") +
  # scale_y_continuous(breaks=seq(0, 1, by = 0.25)) +
  # coord_cartesian(ylim = c(0, 1.0)) +
...
data_text680 <- data.frame(Strain  = c("PE-rich_048"), label = c('OD680'))
data_text720 <- data.frame(Strain  = c("PE-rich_048"), label = c('OD720'))
...
ggplot() + 
  ...
  geom_area(aes(x=x, y=par/max(par)*max(spline)),fill="tan1", alpha=0.3, df) +
  geom_vline(xintercept = 149, linetype = "dotdash", col="brown1", lwd=0.7) + 
  geom_line(aes(x = time_h, y = meanfittedDeltaOD_h), colour = "blue", size = 0.8, alpha = 0.9, show.legend = F, MCGrowthRateFitData48) +
  geom_line(aes(x = time_h, y = meanDeltaOD_h, colour = as.factor(Strain)), size = 0.7, alpha = 0.9, show.legend = F, MCGrowthRateFitData48) +
  geom_line(aes(x = time_h, y = meanOD720_h, colour = as.factor(Strain)), size = 0.7, alpha = 0.9, show.legend = F, MCGrowthRateFitData48) +
  geom_line(aes(x = time_h, y = meanOD680_h, colour = as.factor(Strain)), size = 0.7, alpha = 0.9, show.legend = F, MCGrowthRateFitData48) +
  
  geom_text(data=data_textExp, aes(x=72, y=1.0, label=label), size=4.3) +
  geom_text(data=data_textSt, aes(x=240, y=1.0, label=label), size=4.3) +
  
  geom_text(data=data_text680, aes(x=310, y=0.75, label=label), size=3.5) +
  geom_text(data=data_text720, aes(x=310, y=0.92, label=label), size=3.5) +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_FirstDerivative2",".png",sep = "")), height=4.5, width= 7,  dpi = 300, limitsize = TRUE)
```
