...
ggplot() + 
  ...
  geom_vline(xintercept = 149, col="blue", lwd=0.5) + 
  geom_vline(xintercept = tmx, linetype="dashed", col="black", lwd=0.4) + 
  
  geom_text(data=data_textA, aes(x=336, y=0.9, label=label), size=4.5) +
...
ggplot() + 
  ...
  geom_text(data=data_textAccLen, aes(x=165, y=0.12, label=label), size=3.5) +
  geom_text(data=data_textDecLen, aes(x=182, y=0.12, label=label), size=3.5) +
  geom_text(data=data_textB, aes(x=190, y=0.85, label=label), size=4.5) +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_FirstDerivativeCut",".png",sep = "")), height=4.5, width= 7,  dpi = 300, limitsize = TRUE)
```
