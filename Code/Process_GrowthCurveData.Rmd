...
# plot the results
ggplot(df,aes(x=x,y=spline)) + 
  geom_line(colour="black")  + 
  geom_line(aes(y=s-1),col="tan1",lwd=2) + 
  geom_line(aes(y=yy),col="blue",lwd=1.0)  + 
  geom_line(aes(x=x,y=der*25)) + 
  geom_area(aes(y=par/max(par)*max(spline)),fill="tan1", alpha=0.3) +
  geom_vline(xintercept = 149, col="darkgreen", lwd=1.1) + 
  geom_vline(xintercept = tmx, linetype="dashed", col="#a02a2a", lwd=.9) + 
  scale_y_continuous(sec.axis = sec_axis( trans=~./25, name="1st derivative of OD680")) + 
...
colnames(outputData) <- c("nn", "Strain", "Par_ue", "Photoperiod[h]", "start[step]", "end[step]", "Tstart[minutes]", "Tend[minutes]", "OD680start", "OD680end", "log10(OD680start)", "log10(OD680end)", "logDiff", "Diff")
...
```{r save rds}
 saveRDS(object = outputData, file = file.path(DataOut, paste(fnameFileName,  "20211214_PICO_MC247_RUN39_hourlyGrowth.Rds",  sep = "_")), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
