...
tMaxAGLines_056<-tmaxAG %>% 
    dplyr::filter(Strain == "PC-rich_056") %>% 
    dplyr::select(c(Par_ue, Photoperiod, facetsPar_ue, facetsPhotoperiod, tMaxAG)) %>% 
  unique()
tMaxAGLines_077<-tmaxAG %>% 
    dplyr::filter(Strain == "PC-rich_077") %>% 
    dplyr::select(c(Par_ue, Photoperiod, facetsPar_ue, facetsPhotoperiod, tMaxAG)) %>% 
  unique()
tMaxAGLines_048<-tmaxAG %>% 
    dplyr::filter(Strain == "PE-rich_048") %>% 
    dplyr::select(c(Par_ue, Photoperiod, facetsPar_ue, facetsPhotoperiod, tMaxAG)) %>% 
  unique()
tMaxAGLines_127<-tmaxAG %>% 
    dplyr::filter(Strain == "PE-rich_127") %>% 
    dplyr::select(c(Par_ue, Photoperiod, facetsPar_ue, facetsPhotoperiod, tMaxAG)) %>% 
  unique()
```
...
OD680<-MultiCultiDataAll %>%
  ...
  filter(Run != 77) %>% 
  ggplot() +
  geom_area(aes(x = Time_h/24, y = meanActinic_par_h), size = 0.1, fill = "tan1", alpha = 0.3) +
  geom_line(aes(x = Time_h/24, y = meanOD680_h, colour = as.factor(Strain)), size = 0.5, show.legend = T) +
  geom_vline(data = tMaxAGLines_056, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "seagreen4", size=0.4) +
  geom_vline(data = tMaxAGLines_077, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "palegreen3", size=0.4) +
  geom_vline(data = tMaxAGLines_048, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "brown4", size=0.4) +
  geom_vline(data = tMaxAGLines_127, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "brown1", size=0.4) +
  geom_text(data=data_textA, aes(x=2, y=0.75, label=label), size=6) +
  
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="") +
  scale_y_continuous(breaks=seq(0, 1, by = 0.25)) +
  scale_x_continuous(breaks=seq(0, 15, by = 3)) +
  coord_cartesian(ylim = c(0, 1.0), xlim = c(0, 14)) +
...
OD720<-MultiCultiDataAll %>%
  #filter(Par_ue != 600) %>% 
  filter(Run != 77) %>% 
ggplot() +
  geom_area(aes(x = Time_h/24, y = meanActinic_par_h), size = 0.1, fill = "tan1", alpha = 0.3) +
  geom_line(aes(x = Time_h/24, y = meanOD720_h, colour = as.factor(Strain)), size = 0.5, show.legend = T) +
  geom_vline(data = tMaxAGLines_056, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "seagreen4", size=0.4) +
  geom_vline(data = tMaxAGLines_077, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "palegreen3", size=0.4) +
  geom_vline(data = tMaxAGLines_048, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "brown4", size=0.4) +
  geom_vline(data = tMaxAGLines_127, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "brown1", size=0.4) +
  geom_text(data=data_textB, aes(x=2, y=0.75, label=label), size=6) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="") +
  scale_y_continuous(breaks=seq(0, 1, by = 0.25)) +
  scale_x_continuous(breaks=seq(0, 15, by = 3)) +
  coord_cartesian(ylim = c(0, 1.0), xlim = c(0, 14)) +
  labs(y = "Optical density ("~OD[720]~")", x = "Elapsed time (d)") +
  #ggh4x::facet_nested(cols = vars(facetsPhotoperiod, Photoperiod), rows = vars(facetsPar_ue, Par_ue), labeller = label_parsed) +
  ggh4x::facet_nested(cols = vars(facetsPhotoperiod, Photoperiod), rows = vars(facetsPar_ue, factor(Par_ue, levels=c("900","600","300","180","90","30"))), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
...
ODDelta<-MultiCultiDataAll %>%
  #filter(Par_ue != 600) %>% 
  filter(Run != 77) %>% 
  ggplot() +
  geom_area(aes(x = Time_h/24, y = meanActinic_par_h), size = 0.1, fill = "tan1", alpha = 0.3) +
  geom_line(aes(x = Time_h/24, y = meanDeltaOD_h, colour = as.factor(Strain)), size = 0.5, show.legend = T) +
  geom_vline(data = tMaxAGLines_056, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "seagreen4", size=0.4) +
  geom_vline(data = tMaxAGLines_077, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "palegreen3", size=0.4) +
  geom_vline(data = tMaxAGLines_048, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "brown4", size=0.4) +
  geom_vline(data = tMaxAGLines_127, aes(xintercept = tMaxAG/24), linetype="dotdash", colour = "brown1", size=0.4) +
  geom_text(data=data_textA, aes(x=2, y=0.3, label=label), size=6) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="") +
  scale_y_continuous(breaks=seq(0, 0.4, by = 0.1)) +
  scale_x_continuous(breaks=seq(0, 15, by = 3)) +
  coord_cartesian(ylim = c(0, 0.4), xlim = c(0, 14)) +
  labs(y = "Optical density (chlorophyll proxy"~OD[680]~"-"~OD[720]~")", x = "Elapsed time (d)") +
...
```{r create GrowthCurve plot, fig.height = 8, fig.width = 8, warning = FALSE}
# MCGrowthRateFitData56<-MCGrowthRateFitData %>% 
#   filter(Strain == "PC-rich_056")
# MCGrowthRateFitData77<-MCGrowthRateFitData %>% 
#   filter(Strain == "PC-rich_077")
# MCGrowthRateFitData48<-MCGrowthRateFitData %>% 
#   filter(Strain == "PE-rich_048")
# MCGrowthRateFitData127<-MCGrowthRateFitData %>% 
...
data_textExp <- data.frame(Strain  = c("PE-rich_048"), label = c('Exponential'))
data_textSt <- data.frame(Strain  = c("PE-rich_048"), label = c('Pre-stationary'))
#data_textDelta <- data.frame(Strain  = c("PE-rich_048"), label = c('\u0394 OD'))
...
MMPlot %>% 
ggplot() + 
  geom_line(aes(x=x/24, y=der*25), df) + 
  #geom_line(aes(x = Time_h, y = meanOD680_h, colour = as.factor(Strain)), alpha = 0.9, size = 1, show.legend = F) +
  geom_area(aes(x = Time_h/24, y = meanActinic_par_h*5), size = 0.1, fill = "tan1", alpha = 0.3) +
  #geom_area(aes(x=x/24, y=par/max(par)*max(spline)),fill="tan1", alpha=0.3, df) +
  geom_vline(xintercept = 149/24, linetype = "dotdash", col="brown1", lwd=0.7) + 
  geom_line(aes(x = time_h/24, y = meanfittedDeltaOD_h), colour = "blue", size = 0.8, alpha = 0.9, show.legend = F, MCGrowthRateFitData48) +
  geom_line(aes(x = time_h/24, y = meanDeltaOD_h, colour = as.factor(Strain)), size = 0.7, alpha = 0.9, show.legend = F, MCGrowthRateFitData48) +
  geom_line(aes(x = time_h/24, y = meanOD720_h, colour = as.factor(Strain)), size = 0.7, alpha = 0.9, show.legend = F, MCGrowthRateFitData48) +
  geom_line(aes(x = time_h/24, y = meanOD680_h, colour = as.factor(Strain)), size = 0.7, alpha = 0.9, show.legend = F, MCGrowthRateFitData48) +
  
  geom_text(data=data_textExp, aes(x=72/24, y=1.0, label=label), size=4.3) +
  geom_text(data=data_textSt, aes(x=240/24, y=1.0, label=label), size=4.3) +
  
  #geom_text(data=data_textDelta, aes(x=310, y=0.23, label=label), size=3.5) +
  annotate("text", x = 310/24, y = 0.75, label = "OD[720]", parse = TRUE) +
  annotate("text", x = 310/24, y = 0.92, label = "OD[680]", parse = TRUE) +
  annotate("text", x = 310/24, y = 0.23, label = 'ΔOD', parse = TRUE) +
  
  scale_y_continuous(sec.axis = sec_axis( trans=~./25, name="1"^st~"derivative of"~OD[680]~"")) + 
  #scale_x_continuous(breaks=seq(0,360,24)) + 
  scale_x_continuous(breaks=seq(0, 14, by = 1)) +
  coord_cartesian(ylim = c(-0.2, 1.0), xlim = c(0, 14)) +
  labs(y = "Optical density (OD)", x = "Elapsed time (d)") +
  ggh4x::facet_nested(cols = vars(Photoperiod),labeller = labeller(WL = label_both, Photoperiod = Photoperiod.labs)) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
