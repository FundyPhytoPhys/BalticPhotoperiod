...
world_coordinates$facetsPhotoperiod24 <- 'Photoperiod~~(24~h)'
Map24<-ggplot() + 
  ...
  geom_rect(data=d2, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), fill = "gold1", color="gold1", alpha=0.5) +
  geom_text(data=data_textA, aes(x=-190, y=80, label=label), size=6) +
...
d=data.frame(x1=c(-Inf), x2=c(Inf), y1=c(23), y2=c(66))

world_coordinates <- map_data("world") 
world_coordinates$facetsPhotoperiod16 <- 'Photoperiod~~(16~h)'
...
d=data.frame(x1=c(-Inf), x2=c(Inf), y1=c(-23), y2=c(23))

world_coordinates <- map_data("world") 
world_coordinates$facetsPhotoperiod12 <- 'Photoperiod~~(12~h)'
Map12<-ggplot() + 
  geom_map(data = world_coordinates, map = world_coordinates, aes(long, lat, map_id = region), color = "white", fill = "gray50", size = 0.2) +
  geom_rect(data=d, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), fill = "gold1", color="gold1", alpha=0.5) +
...
world_coordinates <- map_data("world") 
world_coordinates$facetsPhotoperiod8 <- 'Photoperiod~~(8~h)'
Map8<-ggplot() + 
  geom_map(data = world_coordinates, map = world_coordinates, aes(long, lat, map_id = region), color = "white", fill = "gray50", size = 0.2) +
  geom_rect(data=d, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), fill = "gold1", color="gold1", alpha=0.5) +
...
Photoperiod24<-MCGrowthRate %>%
  ...
  coord_cartesian(xlim = c(0, 25), ylim = c(0, 900)) +
  labs(x = "Time of day (h)", y = "PAR ( µmol photons "~m^-2~""~s^-1~")") +
  ggh4x::facet_nested(cols = vars(facetsPhotoperiod24), labeller = label_parsed) +
  # ggh4x::facet_nested(cols = vars(facetsPhotoperiod, Photoperiod), rows = vars(facetsPar_ue, factor(Par_ue, levels=c("900","600","300","180","90","30"))), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
...
Growth24<-GrowthRate %>%
  ...
  geom_vline(xintercept = c(1, 2, 3, 4, 5), size = 0.3, linetype = "dotted") +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4")) +
...
```{r, warning = FALSE, fig.height = 16, fig.width = 12}
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_MapCombo",".png",sep = "")), height=16, width= 16,  dpi = 300, limitsize = TRUE)
```
