...
# (ContourGAM_BA127R <- ggplot(na.omit(HyperChlPlotHigh), aes(Photoperiod, Par_ue, z = fit, fill = fit)) + 
#     geom_tile() +
...
(ContourGAM_BA77G <- ggplot(data = na.omit(HyperChlPlotHigh)) + 
    ...
    scale_fill_gradientn(name = bquote("µ ("~ d^-1~")"), colours=c("white", "black"), limits  = range(minMu, 3)) +
