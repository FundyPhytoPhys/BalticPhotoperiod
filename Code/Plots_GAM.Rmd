...
SySl - fixing missing data
# Dummy points
```{r}
PicoGrowthFitsSySl<-PicoGrowthFits %>% 
dplyr::select(-c(Filename, PrimaryOperator, Tube, SampleID, Run, O2_Category, LightShape))

# colnames(PicoGrowthFitsSySl)

# dummy
gs4_deauth()
# this is the URL or ID of a Sheet readable by anyone (with a link)
StartingPointsGAM <- read_sheet("https://docs.google.com/spreadsheets/d/1knWC58evr43qRyXNb4OgHaUrEB5VEo8v1egHss4QDRI/edit#gid=0") 
PicoGrowthFitsSS<-rbind(PicoGrowthFitsSySl, StartingPointsGAM)
...
(ContourGAM_BA77G <- ggplot(data = na.omit(HyperChlPlotHigh)) + 
    ...
    metR::geom_contour2(aes(Photoperiod, Par_ue, z = fit, fill = fit, label = stat(level)),
                        ...
                        skip = 0) +
    metR::geom_contour2(data = PicoGrowthFitsSS, aes(x = Photoperiod, y = Par_ue, z = PARPhotonDose_day), breaks = c(100000,500000,1000000,2000000,4000000,8000000,16000000, 32000000), linetype = "dashed") + 
...
(ContourGAM_BA56Gc <- ggplot(data = na.omit(HyperChlPlotHigh)) + 
    ...
    metR::geom_contour2(aes(Photoperiod, Par_ue, z = fit, fill = fit, label = stat(level)),
                        breaks = ContourLines$quantileValue,
                        skip = 0) +
    metR::geom_contour2(data = PicoGrowthFitsSS, aes(x = Photoperiod, y = Par_ue, z = PARPhotonDose_day), breaks = c(100000,500000,1000000,2000000,4000000,8000000,16000000, 32000000), linetype = "dashed", size = 0.2, colour = "gray10") + 
    scale_x_continuous(limits = c(8, 24), breaks = c(8, 12, 16, 24)) +
    scale_y_continuous(limits = c(10, 900), breaks = c(30, 90, 180, 300, 600, 900)) +
    geom_text(data=data_textA, aes(x=9, y=860, label=label), size=5) +
    labs(x = "Photoperiod (h)", y = "PAR ( µmol photons "~m^-2~""~s^-1~")") +
