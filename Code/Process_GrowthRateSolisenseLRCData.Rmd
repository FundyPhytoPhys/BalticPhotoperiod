...
Solisense %>%
  ...
  filter(Phase == "Exponential") %>% 
  ggplot() +
  geom_point(aes(x = meanCellmL_MDPico, y = JVPSII_Par_ue, colour = as.factor(Strain)),  alpha = 0.8, size = 3, show.legend = T) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="", labels = lab1) +
  ggh4x::facet_nested(cols = vars(Phase, Photoperiod), rows = vars(Par_ue), labeller = label_parsed) +
  theme_bw() 
```
...
GrowthRateSoliExp<-GrowthRateSol445 %>% 
  ...
  mutate(JVPSII_Par_ue_cell = JVPSII_Par_ue/meanCellmL_MDPico) %>% 
  mutate(JVPSII_HalfPar_ue_cell = JVPSII_HalfPar_ue/meanCellmL_MDPico)  
```
...
MCGrowthJVPSII90Fit <- MCGrowthJVPSIISP %>% 
  filter(Par_ue ==90) %>%  
  nest(data = -c("Strain", "facetsStrain")) %>% 
  mutate(GRCplatt_Model = map(data, ~nlsLM(deltaOD_Lmu_corr ~ grcplatt(I = JVPSII_Par_ue_Chl_day, a, b, Pmax), data = .x, start = list(a =0.01/10, b = 1/5, Pmax = 100), lower = c(0, 0, 0)))) %>%      
  mutate(GRCplatt_Param = map(GRCplatt_Model, tidy),          
         GRCplatt_Predict = map(GRCplatt_Model, possibly(augment)),          
         GRCplatt_Glance = map(GRCplatt_Model, possibly(glance)))
...
ggplot() +
  ...
  geom_line(aes(JVPSII_Par_ue_Chl_day, .fitted), colour = "darkslategray", linetype = "dashed", show.legend = F, GRCplatt_PredictGrowth30_BA56) +
  geom_line(aes(JVPSII_Par_ue_Chl_day, .fitted), colour = "indianred3", linetype = "dashed", show.legend = F, GRCplatt_PredictGrowth300_BA56) +
  geom_line(aes(JVPSII_Par_ue_Chl_day, .fitted), colour = "goldenrod2", linetype = "dashed", show.legend = F, GRCplatt_PredictGrowth900_BA56) +
  
  geom_line(aes(JVPSII_Par_ue_Chl_day, .fitted), colour = "goldenrod2", linetype = "dashed", show.legend = F, GRCplatt_PredictGrowth900_BA48) +
  
  geom_line(aes(JVPSII_Par_ue_Chl_day, .fitted), colour = "darkslategray", linetype = "dashed", show.legend = F, GRCplatt_PredictGrowth30_BA127) +
  geom_line(aes(JVPSII_Par_ue_Chl_day, .fitted), colour = "goldenrod2", linetype = "dashed", show.legend = F, GRCplatt_PredictGrowth900_BA127) +
...
xxx - JVPSII need to by multiply - 1e23? 
