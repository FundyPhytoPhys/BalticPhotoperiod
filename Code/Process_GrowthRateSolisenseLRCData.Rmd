...
PlotB<-JVPSIIParamAll_Photoperiod %>% 
  filter(term == "a") %>% 
  filter(Fit!= "All") %>% 
  rename(SE="std.error") %>% 
  ggplot() +
  ...
  geom_ribbon(aes(x = as.numeric(Fit), ymin = estimate - SE, ymax = estimate + SE), alpha = 0.1, width=0, size=0.3, data = . %>% filter(estimate<0.02), show.legend = F) +
  
  geom_text(data=data_textB, aes(x=8, y=0.0019, label=label), size=5) +
...
ggp<- PlotAPhotoperiodJVPSII+PlotB  
ggp                           
```
...
PlotB<-JVPSIIParamAll_Light %>% 
  ...
  geom_errorbar(aes(x = as.numeric(Fit), ymin = estimate - SE, ymax = estimate + SE), width=0, size=0.3, data = . %>% filter(SE<0.02), show.legend = F) +
  geom_text(data=data_textB, aes(x=8, y=0.038, label=label), size=5) +
  #scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4"), name="") +
  scale_y_continuous(breaks=seq(0, 0.04, by = 0.01), label=scientific_10) +
