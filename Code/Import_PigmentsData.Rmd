...
-------------------------------------- Absorbance OD growth ClarioSTAR -------------------------
...
------------------------------------ PAMAS vs. ClarioSTAR correlation -------------------------------------------
...
MCBA48R <- GrowthCurve %>% 
filter(Strain == "PE-rich_048") %>% 
mutate(Chla_ugL_Tur = 4328.492*meanDeltaOD_h+144.24) 

MCBA127R <- GrowthCurve %>% 
filter(Strain == "PE-rich_127") %>% 
mutate(Chla_ugL_Tur = 2805.1*meanDeltaOD_h+183.9)

GrowthCurve_Turner <- rbind(MCBA127R, MCBA48R, MCBA56G, MCBA77G)
...
GrowthCurve_cellmL <- rbind(MCBA127R, MCBA48R, MCBA56G, MCBA77G)
...
```{r save rds}
saveRDS(MC_cellmLPigment_MDPico, file.path(DataOut, paste(Project, "Imported_PigmentsMDPicoTurnerData.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```




------------------------# Estimated Pigments content per cell (pg/cell) - based on PAMAS correlations-----------------------


# Merge growth curve with cell/mL (PAMAS) with pigments content based on Olis
