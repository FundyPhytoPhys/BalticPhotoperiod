...
ggplot(PhycoChlaRatioChoosen, aes(x = PARPhotonDose_day, y = PhycoChlaRatio)) +
  ...
  labs(y="Phycobiliprotein to Chl" ~italic(a)~ "ratio (µg/µg)", x = "PAR dose ( µmol photons "~m^-2~""~d^-1~")") +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_PhycoChlaRatio",".png",sep = "")), height=9, width= 7,  dpi = 300, limitsize = TRUE)
```
...
ggplot(PURPARRatioChoosen, aes(x = PARPhotonDose_day, y = PURPARRatio)) +
  ...
  scale_shape_manual(values = c(15, 16, 17,18), name="", labels = lab1) +
  scale_colour_manual(values = c("darkslategray", "lightblue4", "hotpink4", "indianred3",  "goldenrod2", "khaki2"), name="", labels = lab2) +
  labs(y="PUR/PAR ratio", x = "PAR dose ( µmol photons "~m^-2~""~d^-1~")") +
  scale_x_continuous(breaks=seq(0, 60000000, by = 30000000), label=scientific_10) +
