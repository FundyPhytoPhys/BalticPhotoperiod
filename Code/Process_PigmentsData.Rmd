...
MCPigmentFile <- "../Data/ImportedData/ImportedPigmentsData/BalticPhotoperiod_Imported_PigmentData.Rds"
...
# I have 24 measurements per pigment per day (b/c number of cells changes every h and thus pigments per cell changed as well)
...
```{r save plot}
ggsave(file = file.path(PlotsPath, paste("SFig_Pigments",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
...
OLISAllPURTidyFile <- "../Data/ProcessedData/ProcessedOlisJazData/BalticPhotoperiod_Processed_OlisSpectraTidy.Rds"
...
```{r}
saveRDS(MCPigmentPlot, file.path(FigRdsPath, paste(Project, "Fig_Pigment_1.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)

saveRDS(MCPigmentRatioPlot, file.path(FigRdsPath, paste(Project, "Fig_PigmentRatioA_1.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)

saveRDS(OLISAllPURTidy, file.path(FigRdsPath, paste(Project, "Fig_PigmentRatioB_2.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
...
MCPigmenttMaxAG <- MCPigment %>%
  left_join(., tmaxAG, by = c("Strain"="Strain","Par_ue"="Par_ue","Photoperiod"="Photoperiod", "WL"="WL", "O2"="O2", "PARPhotonDose_day"="PARPhotonDose_day", "facetsPar_ue"="facetsPar_ue", "facetsPhotoperiod"="facetsPhotoperiod"))
```
...
# Chosen only one pigment data per day - taken at noon (12.00 PM)

```{r}
MCPigmenttMaxAGChoosen <- MCPigmenttMaxAG %>%
  filter(ToD == 12)
```
...
MCPigmenttMaxAGChoosenExp <-rbind(O1, O2, O3, O4, O5, O6, O7, O8, O9, O10, O11, O12, O13, O14, O15, O16, O17)

rm(O1, O2, O3, O4, O5, O6, O7, O8, O9, O10, O11, O12, O13, O14, O15, O16, O17)
```

```{r}
MCPigmenttMaxAGChoosenExp<-MCPigmenttMaxAGChoosenExp %>%
  ...
  # select(c(Strain, Par_ue, Photoperiod, ChlapgcellExp, PhycopgcellExp, PhycoChlaRatioExp, PURPARRatioExp, cellL_OD680Exp, E_daysExp, Time_hExp))
...
```{r}
rm(tMaxAGLines_056, tMaxAGLines_077, tMaxAGLines_048, tMaxAGLines_127, MCPigment, tmaxAG, tmaxAGSelect, MCPigmenttMaxAGChoosen)
```

# Save rds for further analysis

```{r save rds}
saveRDS(MCPigmenttMaxAG, file.path(DataOut, paste(Project, "Processed_PigmentsAll.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)

saveRDS(MCPigmenttMaxAGChoosenExp, file.path(DataOut, paste(Project, "Processed_PigmentsExp.Rds", sep = "_"), fsep = .Platform$file.sep))
```
...
```{r}
colnames(MCPigmenttMaxAGChoosenExp)
```
