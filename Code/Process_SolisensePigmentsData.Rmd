...
stats zrobic z dodawaniem modeli - simple and more complex - to powinno dzialac
```{r}
...
#   filter(Ex_WL == 590) %>% 
#   filter(Strain == "PC-rich_056")
#   SolFitsBA56Exp <- SolFitsBA56Exp[-c(300:310), ]
# 
# SolFitsBA77Exp<-SolFitsAllPigmentPCPE %>%
#   filter(Phase == "Exponential") %>% 
#   filter(Ex_WL == 590) %>% 
#   filter(Strain == "PC-rich_077") 
#   SolFitsBA77Exp <- SolFitsBA77Exp[-c(300:330), ]
# 
# SolFitsBA48Exp<-SolFitsAllPigmentPCPE %>%
#   filter(Phase == "Exponential") %>% 
#   filter(Ex_WL == 590) %>% 
#   filter(Strain == "PE-rich_048") 
#   SolFitsBA48Exp <- SolFitsBA48Exp[-c(300:312), ]
# 
# SolFitsBA127Exp<-SolFitsAllPigmentPCPE %>%
#   filter(Phase == "Exponential") %>% 
#   filter(Ex_WL == 590) %>% 
#   filter(Strain == "PE-rich_127") 
#   SolFitsBA127Exp <- SolFitsBA127Exp[-c(300:318), ]
# 
# SolFitsBA127Exp <- SolFitsBA127Exp %>%  
...
#          Linear_Model_Predict = map(Linear_Model, possibly(augment)),          
#          Linear_Model_Glance = map(Linear_Model, possibly(glance))) %>% 
#   select(c(Linear_Model))
# 
# SolFitsBA48Exp <- SolFitsBA48Exp %>%  
#   drop_na(Sig_nm2psii, AllmeanPhycoChlaRatio) %>% 
#   select(c(Strain, Par_ue, Photoperiod, Ex_WL, meanSig_nm2psii, AllmeanPhycoChlaRatio, O2, WL, Phase, facetsExcitation, facetsStrain, facetsPhase)) %>%  
#   nest(data = -c("Strain", "Ex_WL", "Phase", "facetsPhase", "facetsStrain")) %>%  
#   mutate(Linear_Model = map(data, ~lm(meanSig_nm2psii ~ AllmeanPhycoChlaRatio, data = .x))) %>%    
...
#   select(c(Linear_Model))
# 
# AnovaTest_SigLM<-data.frame(unclass(summary(SolFitsBA127Exp, SolFitsBA48Exp)), check.names = FALSE, stringsAsFactors = FALSE)
#   AAAAAnova<-anova(SolFitsBA127Exp, SolFitsBA48Exp)
# 
#   AAAAAnova<-anova(SolFitsBA127Exp[[1]][[1]], SolFitsBA48Exp[[1]][[1]]) #this give me p = NA
# 
# 
#   
#   
#   
# TestModel590 <- na.omit(TestModel590)
# TestModel445<-TestModel %>% 
#   select(c(Strain, Ex_WL, Phase, Linear_Model)) %>% 
#   filter(Ex_WL == 445)
# 
# TestModel590<-TestModel %>% 
#   select(c(Strain, Ex_WL, Phase, Linear_Model)) %>% 
#   filter(Ex_WL == 590)
# 
#   anova056077St<-anova(TestModel590[[4]][[1]], TestModel590[[4]][[2]])
#   anova056077St$FitComparison <- '056_077'
# 
# 
# 
