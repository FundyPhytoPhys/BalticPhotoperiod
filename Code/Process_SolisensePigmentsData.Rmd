...
```{r}
  ...
  rm(SolFitsExp, SolFitsSt, SolFitsExp1s, SolFitsSt1s, SolFits, SolFits1s)
```
...
ggplot(SolFitsExpFitAll590, aes(x = PARPhotonDose_day, y = Sig_nm2psii)) +
  ...
  geom_text(data=data_text_AnovaExp, aes(x=12500000, y=0.5, label=label), colour = "blue", size=3.0) +
  geom_text(data=data_text_AnovaSt, aes(x=12500000, y=0.5, label=label), colour = "blue", size=3.0) +
...
# 
# #Comparing model for strains from St phase
# fittedmodelStrainSt<-fittedmodel445
#   anova056077St<-anova(fittedmodelStrainSt[[3]][[5]], fittedmodelStrainSt[[3]][[6]])
#   anova056077St$FitComparison <- '056_077'
#   anova048127St<-anova(fittedmodelStrainSt[[3]][[7]], fittedmodelStrainSt[[3]][[8]])
#   anova048127St$FitComparison <- '048_127'
...
#   anova077127St<-anova(fittedmodelStrainSt[[3]][[6]], fittedmodelStrainSt[[3]][[8]])
#   anova077127St$FitComparison <- '077_127'
#   anovaStrainSt445<-rbind(anova056077St, anova048127St, anova056048St, anova077048St, anova056127St, anova077127St)
#   anovaStrainSt445 <-anovaStrainSt445 %>% 
#   drop_na(Df) 
#   rm(anova056077St, anova048127St, anova056048St, anova077048St, anova056127St, anova077127St)
#   
#   
# #Comparing model for each strains from different phase (Exp vs St)
# fittedmodelPhase<-fittedmodel445
#   anova056ExpSt<-anova(fittedmodelPhase[[3]][[1]], fittedmodelPhase[[3]][[5]])
#   anova056ExpSt$FitComparison <- '056_Exp_St'
#   anova077ExpSt<-anova(fittedmodelPhase[[3]][[2]], fittedmodelPhase[[3]][[6]])
#   anova077ExpSt$FitComparison <- '077_Exp_St'
#   anova048ExpSt<-anova(fittedmodelPhase[[3]][[3]], fittedmodelPhase[[3]][[7]])
#   anova048ExpSt$FitComparison <- '048_Exp_St'
#   anova127ExpSt<-anova(fittedmodelPhase[[3]][[4]], fittedmodelPhase[[3]][[8]])
#   anova127ExpSt$FitComparison <- '127_Exp_St'
#   anovaPhase445<-rbind(anova056ExpSt, anova077ExpSt, anova048ExpSt, anova127ExpSt)
#   anovaPhase445 <-anovaPhase445 %>% 
#   drop_na(Df) 
#   rm(anova056ExpSt, anova077ExpSt, anova048ExpSt, anova127ExpSt)
...
MCPigmentAllFile <- "../Data/ProcessedData/ProcessedPigmentsData/Baltic_Photoperiod_Processed_PigmentAll.Rds"
...
# Altrough MCPigmentAll already contain Exp and St phase, I took exactly the same parameters (settings) like Sig, b/c MCPigmentAll do not contain information on 600 uE. And for one case I tokk date from very early st phase.
...
# Filter only Exponential or Stationary data

```{r}
# MCPigmentAll<-MCPigmentAll %>%
#   select(-c(Phase)) # Removed previous Phase b/c do not contained 600uE data
# 
# MCPigmentAllExp<-MCPigmentAll %>%
#   filter(Phase == "Exponential")
# 
# MCPigmentAllSt<-MCPigmentAll %>%
#   filter(Phase == "Pre-stationary")
```
...
```{r}
rm(tmaxAG, tMaxAGLines_056, tMaxAGLines_077, tMaxAGLines_048, tMaxAGLines_127)

#MCPigmentAllExp, MCPigmentAllSt
```
...
SolFitsAllPigmentfitPhycoChlalinearStrain <- SolFitsAllPigmentPCPE %>%  
  select(c(Strain, Par_ue, Photoperiod, Ex_WL, meanSig_nm2psii, AllmeanPhycoChlaRatio, O2, WL, Phase, facetsExcitation, facetsStrain)) %>%  
...
augmented590pigment<-SolFitsAllPigmentfitPhycoChlalinearStrain  %>%
  unnest(Linear_Model_Predict) %>%
  select(c(Strain, Ex_WL, meanSig_nm2psii, AllmeanPhycoChlaRatio, Phase, .fitted)) %>% 
  filter(Ex_WL == 590)
...
```{r save plot}
ggsave(file = file.path(PlotsPath, paste("SigmavsPigments590Phasetest_SupPlot",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
