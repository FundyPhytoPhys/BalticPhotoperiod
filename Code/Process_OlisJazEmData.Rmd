...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_OlisSpectra",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
...
OLISSpectraMetaPURShort %>%
  ...
  geom_area(aes(x = nm, y = PURNormBA127R, fill = "brown4"), alpha = 0.5, show.legend = F) +
  geom_area(aes(x = nm, y = PURNormBA77G, fill = "palegreen3"), alpha = 0.5, show.legend = F) +
  geom_line(aes(x = nm, y = AbsNorm440, colour = as.factor(Strain), linetype = as.factor(Strain)), show.legend = F, linewidth = 0.7) +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("SFig_OlisSpectra_Photoperiod",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
...
OLISSpectraMetaPUR %>%
  ggplot() +
  geom_area(aes(x = nm, y = EmNormJaz439), fill = "gray84", alpha = 0.6, show.legend = F) +
  geom_line(aes(x = nm, y = AbsNorm440, colour = as.factor(Strain)), show.legend = F, linewidth = 0.7) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4")) +
  stat_wl_strip(aes(x = nm), ymin = -Inf, ymax = -0.025, alpha = 0.5) +
  scale_fill_identity() +
  scale_y_continuous(breaks=seq(0, 3, by = 1)) +
  coord_cartesian(ylim = c(0, 3)) +
  labs(y = "Normalized absorbance", x = "Wavelength (nm)") +
  ggh4x::facet_nested(cols = vars(Par_ue), rows = vars(Strain), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        axis.text = element_text(size=12),
