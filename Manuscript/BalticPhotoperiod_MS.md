...
All analysis of obtained results was conducted using R version 4.3.0 [@rcoreteamLanguageEnvironmentStatistical2019] running under RStudio [@teamrstudio.RStudioIntegratedDevelopment2015]. To determine significant differences in studied experiments the “stats” v. 3.6.2 R standard packages were used. This package provides basic statistical functions, including the *lm()* function for linear regression, *aov()* function for ANOVA, and *t.test()* function for t-test. The *SSasymp()* function (Self-Starting Nls Asymptotic Regression Model) was used to perform a single phase exponential decay fit model [Citationxxx]. A modified Levenberg-Marquardt fitting algorithm [@minpack.lm] was used for estimating logistic fits of chlorophyll proxy OD~680~-OD~720~ vs. elapsed time for each combination of strain, photoperiod, and peak PAR.  
