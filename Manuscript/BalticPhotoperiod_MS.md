...
We calibrated the *JV*~PSII~ estimator to absolute rates of electron transport (Eq. (4)) using parallel measures of oxygen evolution (µmolO~2~ L^−1^ s^−1^), captured simultaneously to the FRRf measures, below light saturation, using a FireSting robust oxygen probe (PyroScience, Germany) inserted in the cuvette for some RLC runs (Fig. <a href="#fig:LIFT">3</a>). For the  blue LED (Ex~445nm~) excitation we used a calibration slope of 106941, while for orange LED (Ex~590nm~) excitation we used a calibration slope of 251096.
